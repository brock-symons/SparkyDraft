...
function drawGrid(ctx, view, w, h, drawing) {
  ...
  function lines(step, color, width) {
    ...
    for (let wx = startWorldX; ; wx += step) {
      ...
      if (x > w) break;
      if (x >= 0) { ctx.moveTo(Math.round(x) + 0.5, 0); ctx.lineTo(Math.round(x) + 0.5, h); }
    }
    ...
    for (let wy = startWorldY; ; wy += step) {
      ...
      if (y > h) break;
      if (y >= 0) { ctx.moveTo(0, Math.round(y) + 0.5); ctx.lineTo(w, Math.round(y) + 0.5); }
    }
    ctx.stroke();
  }
...
function drawOrigin(ctx, view, w, h, drawing) {
  ...
  if (o.x >= -50 && o.x <= w + 50) {
    ctx.beginPath(); ctx.strokeStyle = PAINT.originY; ctx.lineWidth = 1;
    ctx.moveTo(Math.round(o.x) + 0.5, 0); ctx.lineTo(Math.round(o.x) + 0.5, h); ctx.stroke();
  }
  if (o.y >= -50 && o.y <= h + 50) {
    ctx.beginPath(); ctx.strokeStyle = PAINT.originX; ctx.lineWidth = 1;
    ctx.moveTo(0, Math.round(o.y) + 0.5); ctx.lineTo(w, Math.round(o.y) + 0.5); ctx.stroke();
  }
}
...
function drawSnapGuides(ctx, view, guides, w, h) {
  ...
  for (const g of guides) {
    ctx.beginPath();
    ctx.lineWidth = 1;
    if (g.kind === 'device') { ctx.strokeStyle = PAINT.snapDevice; ctx.setLineDash([]); }
    else if (g.kind === 'wall') { ctx.strokeStyle = PAINT.snapWall; ctx.setLineDash([6, 4]); }
    else { ctx.strokeStyle = PAINT.snapGrid; ctx.setLineDash([2, 4]); }
    if (g.axis === 'x') {
...
function drawSelectionBounds(ctx, view, bounds) {
  ...
  const x = a.x - pad, y = a.y - pad;
  const w = (b.x - a.x) + pad * 2, h = (b.y - a.y) + pad * 2;
  ctx.strokeStyle = PAINT.selectionSoft;
  ctx.lineWidth = 1;
  ctx.setLineDash([4, 4]);
  ctx.strokeRect(Math.round(x) + 0.5, Math.round(y) + 0.5, Math.round(w), Math.round(h));
...
function drawMeasure(ctx, view, measure, scale, formatDistance) {
  ...
  for (const p of [a, b]) {
    ...
    ctx.fillStyle = PAINT.measure; ctx.fill();
  }
...
function drawPlacementGhost(ctx, view, ghost, sym) {
  ...
  ctx.fillStyle = sym.color + '33'; ctx.fill();
  ctx.strokeStyle = sym.color; ctx.lineWidth = 1.5; ctx.setLineDash([4, 3]);
